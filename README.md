...
### Private local deployment helper

For this machine's local source build, run:

```bash
bash scripts/deploy-local.sh
```

It walks through every `.env` field without printing its full current value: it shows a short beginning-and-end mask, and **Enter keeps the existing value**. Empty session and payment-webhook secrets can be entered or generated by typing `generate`. The helper writes `.env` with owner-only permissions, rejects incomplete payment or Tuya configuration, rebuilds the loopback-only container on port `8089`, waits for its health check, and configures tailnet-only Tailscale Serve. It never enables Tailscale Funnel.
