...
window.openTxModal = async function(type, prefilledCategory = null, prefilledQty = null) {
    ...
    const renderInputs = () => {
         ...
         let html = '';
         let currentTotalUnsold = 0;
         if (type === 'sale') {
              const selectorContainer = document.createElement('div');
              appendSaleCustomerSelector(selectorContainer, store.customers || []);
              html += selectorContainer.innerHTML;

              if (cat === 'eggs') {
                   const inventoryAging = computeEggInventoryAging(logs, txs, stagingToday);
                   currentTotalUnsold = inventoryAging.totalUnsold;
         ...
         if (type !== 'return' && type !== 'write_off' && cat === 'feed') {
    ...
    $('tx-form').addEventListener('submit', async (e) => {
        ...
        const selectedCustomerId = $('tx-customer') ? $('tx-customer').value : '';
        const selectedCustomer = (store.customers || []).find(c => String(c.id) === String(selectedCustomerId)) || null;
        const customerFields = saleCustomerFields(selectedCustomer);

        if (!canSaveSaleForCustomer(customerFields)) {
            window.showToast('Selected customer payment terms require a valid customer record.', 'danger');
            return;
        }
        ...
        const newTx = {
            ...
            ...(type === 'sale' ? {
                ...
                buyerName: customerFields.buyerName,
                buyerTerms: customerFields.buyerTerms,
                paymentTermsDays: customerFields.paymentTermsDays,
                route
            } : { route }),
            defectType,
            reason,
            status: (type === 'sale' && customerFields.buyerTerms !== 'COD') ? 'unpaid' : 'paid',
        ...
        try {
            ...
            console.error('Transaction save failed:', error.message, 'status:', error.status);
            window.showToast(`Transaction was not saved: ${error.message}`, 'danger');
            return;
        }
