...
*   **Concurrency Write Lock**: Set `PRAGMA busy_timeout = 5000;` to prevent database lockups and transient errors during parallel operational writes or client sync requests.
*   **Ledger Idempotency**: Added a unique index constraint `idx_ledger_ref_type_id` on the `(ref_type, ref_id)` fields of `ledger_transactions` to reject duplicate ledger references.
*   **Atomic Transactions**: Wrapped double-entry general ledger transactions and internal ledger sync logic inside database transaction blocks (`BEGIN TRANSACTION` / `COMMIT` / `ROLLBACK`) to eliminate half-written ledger states.
...
#### 6. Historical E2E Test Suite & Infrastructure Hardening (June 2026)
...
*   **Test Auth Race Fix**: The historical runner queried `/api/auth/me` instead of racing on `#nav-dashboard` (which exists immediately in the static HTML structure prior to JS execution).
*   **Superseded target model**: Batch 18A replaced the old arbitrary-target runner with a temporary-copy, generated-credential, loopback-only harness. It cannot target production or user data.
*   **Dedicated Test Account Seeding**: When `E2E_TEST_PASSWORD` is configured, a startup/setup seed ensures a dedicated `e2e_tester` account exists with the `admin` role and `must_change_password = 0` (so it never prompts mid-test). The seed is optional and never blocks normal farm startup.
*   **Historical authentication defaults**: The old runner removed hardcoded default credentials. The current harness generates isolated credentials for every run.
...
*   **Modular Backend Services**: Split the monolithic `server.js` into decoupled, specialized service layers under `services/`:
    ...
    *   [ledger.js](file:///home/kelvitz/AntigravityProjects/PoultryProject/services/ledger.js): Double-entry ledger journal entry logic for farm transactions.
*   **Modular Frontend View Controllers**: Split the massive `js/app.js` into distinct, view-specific modules under `js/` with clean interface routing and a unified state store:
    ...
    *   [settings.js](file:///home/kelvitz/AntigravityProjects/PoultryProject/js/settings.js): Buyer CRM and system users management.
...
### 2. Server Out-of-Band Administration
