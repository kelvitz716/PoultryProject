...
const {
  ...
  sourceTreeDigest,
  writeEvidence
} = require('../playwright/batch18c1');

test('Batch 18C.1 owns its dedicated finance evidence root and refuses targets', () => {
  assert.match(EVIDENCE_ROOT, /evidence[\\/]playwright[\\/]batch18c1$/);
  assert.throws(() => assertSafeConfiguredOrigins({}, ['https://example.test']), /accepts no command-line target/);
  assert.throws(() => assertSafeConfiguredOrigins({ BATCH18C1_ORIGIN: 'https://example.test' }, []), /refuses non-loopback/);
  assert.throws(() => assertSafeConfiguredOrigins({ BATCH18C1_ORIGIN: 'http://127.0.0.1:9999' }, []), /does not accept/);
});

test('Batch 18C.1 source-tree digest is deterministic and excludes runtime data', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'batch18c1-digest-'));
  try {
...
test('Batch 18C.1 results and manifest require the recorded copied-app digest', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'batch18c1-evidence-'));
  const digest = { algorithm: 'sha256', digest: 'a'.repeat(64), file_count: 1 };
  try {
    const screenshot = expectedScreenshotRelativePaths('01 settings customer creation');
